...
from helper import tools, logger, optim
...
def _add_arguments_for_module(parser,
    ...
    for argname in full_args:
        ...
        # If a default parameter can be inferred from the module, pick that one
            ...
            parser.add_argument("--%s" % sub_arg_name, type=argtype, default=default_args_dict[argname])
...
def _parse_arguments():
    ...
    add("--sequence_length", type=int, default=4)
    
    add("--finetuning", type=tools.str2bool, default=False)
    add("--calculate_disparity_scale", type=tools.str2bool, default=False)
    add("--correlation_cuda_enabled", type=tools.str2bool, default=False)
    add("--conv_padding_mode", type=str, default="zeros", choices=["zeros", "replicate", "reflect"])

    add("--save_out", type=tools.str2bool, default=False)
    add("--save_vis", type=tools.str2bool, default=False)
    ...
    _add_arguments_for_module(
        ...
        exclude_classes=["_*"],
        exclude_params=["self","args"],
        forced_default_types={"resize": tools.str2intlist})
